// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Design implementation internals
// See Vtb_uart_tx.h for the primary calling header

#include "Vtb_uart_tx__pch.h"
#include "Vtb_uart_tx___024root.h"

VL_ATTR_COLD void Vtb_uart_tx___024root___eval_static__TOP(Vtb_uart_tx___024root* vlSelf);

VL_ATTR_COLD void Vtb_uart_tx___024root___eval_static(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root___eval_static\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Body
    Vtb_uart_tx___024root___eval_static__TOP(vlSelf);
    vlSelfRef.__Vtrigprevexpr___TOP__tb_uart_tx__DOT__clk__0 
        = vlSelfRef.tb_uart_tx__DOT__clk;
    vlSelfRef.__Vtrigprevexpr___TOP__tb_uart_tx__DOT__rst_n__0 
        = vlSelfRef.tb_uart_tx__DOT__rst_n;
    vlSelfRef.__Vtrigprevexpr___TOP__tb_uart_tx__DOT__tx_done__0 
        = vlSelfRef.tb_uart_tx__DOT__tx_done;
}

VL_ATTR_COLD void Vtb_uart_tx___024root___eval_static__TOP(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root___eval_static__TOP\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Body
    vlSelfRef.tb_uart_tx__DOT__tick_counter = 0U;
}

VL_ATTR_COLD void Vtb_uart_tx___024root___eval_final(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root___eval_final\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
}

#ifdef VL_DEBUG
VL_ATTR_COLD void Vtb_uart_tx___024root___dump_triggers__stl(Vtb_uart_tx___024root* vlSelf);
#endif  // VL_DEBUG
VL_ATTR_COLD bool Vtb_uart_tx___024root___eval_phase__stl(Vtb_uart_tx___024root* vlSelf);

VL_ATTR_COLD void Vtb_uart_tx___024root___eval_settle(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root___eval_settle\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Init
    IData/*31:0*/ __VstlIterCount;
    CData/*0:0*/ __VstlContinue;
    // Body
    __VstlIterCount = 0U;
    vlSelfRef.__VstlFirstIteration = 1U;
    __VstlContinue = 1U;
    while (__VstlContinue) {
        if (VL_UNLIKELY(((0x64U < __VstlIterCount)))) {
#ifdef VL_DEBUG
            Vtb_uart_tx___024root___dump_triggers__stl(vlSelf);
#endif
            VL_FATAL_MT("tb/tb_uart_tx.sv", 3, "", "Settle region did not converge.");
        }
        __VstlIterCount = ((IData)(1U) + __VstlIterCount);
        __VstlContinue = 0U;
        if (Vtb_uart_tx___024root___eval_phase__stl(vlSelf)) {
            __VstlContinue = 1U;
        }
        vlSelfRef.__VstlFirstIteration = 0U;
    }
}

#ifdef VL_DEBUG
VL_ATTR_COLD void Vtb_uart_tx___024root___dump_triggers__stl(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root___dump_triggers__stl\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Body
    if ((1U & (~ vlSelfRef.__VstlTriggered.any()))) {
        VL_DBG_MSGF("         No triggers active\n");
    }
    if ((1ULL & vlSelfRef.__VstlTriggered.word(0U))) {
        VL_DBG_MSGF("         'stl' region trigger index 0 is active: Internal 'stl' trigger - first iteration\n");
    }
}
#endif  // VL_DEBUG

VL_ATTR_COLD void Vtb_uart_tx___024root___stl_sequent__TOP__0(Vtb_uart_tx___024root* vlSelf);
VL_ATTR_COLD void Vtb_uart_tx___024root____Vm_traceActivitySetAll(Vtb_uart_tx___024root* vlSelf);

VL_ATTR_COLD void Vtb_uart_tx___024root___eval_stl(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root___eval_stl\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Body
    if ((1ULL & vlSelfRef.__VstlTriggered.word(0U))) {
        Vtb_uart_tx___024root___stl_sequent__TOP__0(vlSelf);
        Vtb_uart_tx___024root____Vm_traceActivitySetAll(vlSelf);
    }
}

VL_ATTR_COLD void Vtb_uart_tx___024root___stl_sequent__TOP__0(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root___stl_sequent__TOP__0\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Body
    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_tx_done = 0U;
    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_tx = vlSelfRef.tb_uart_tx__DOT__tx;
    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_bit_count 
        = vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_bit_count;
    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_count 
        = vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_count;
    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_state 
        = vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_state;
    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_data_reg 
        = vlSelfRef.tb_uart_tx__DOT__dut__DOT__data_reg;
    if ((2U & (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_state))) {
        if ((1U & (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_state))) {
            if (vlSelfRef.tb_uart_tx__DOT__s_tick) {
                if ((0xfU == (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_count))) {
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_tx_done = 1U;
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_state = 0U;
                } else {
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_state = 3U;
                }
                if ((0xfU != (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_count))) {
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_count 
                        = (0xfU & ((IData)(1U) + (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_count)));
                }
            }
        } else if (vlSelfRef.tb_uart_tx__DOT__s_tick) {
            if ((0xfU == (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_count))) {
                vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_count = 0U;
                if ((7U == (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_bit_count))) {
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_state = 3U;
                }
            } else {
                vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_count 
                    = (0xfU & ((IData)(1U) + (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_count)));
                vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_state = 2U;
            }
        }
        vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_tx 
            = ((1U & (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_state)) 
               || (1U & (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__data_reg)));
        if ((1U & (~ (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_state)))) {
            if (vlSelfRef.tb_uart_tx__DOT__s_tick) {
                if ((0xfU == (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_count))) {
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_bit_count 
                        = (7U & ((IData)(1U) + (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_bit_count)));
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_data_reg 
                        = (0xffU & VL_SHIFTR_III(8,8,32, (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__data_reg), 1U));
                }
            }
        }
    } else {
        if ((1U & (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_state))) {
            vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_tx = 0U;
            if (vlSelfRef.tb_uart_tx__DOT__s_tick) {
                if ((0xfU == (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_count))) {
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_bit_count = 0U;
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_count = 0U;
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_state = 2U;
                } else {
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_count 
                        = (0xfU & ((IData)(1U) + (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_count)));
                    vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_state = 1U;
                }
            }
        } else if (vlSelfRef.tb_uart_tx__DOT__tx_start) {
            vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_count = 0U;
            vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_state = 1U;
        } else {
            vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_state = 0U;
        }
        if ((1U & (~ (IData)(vlSelfRef.tb_uart_tx__DOT__dut__DOT__current_state)))) {
            if (vlSelfRef.tb_uart_tx__DOT__tx_start) {
                vlSelfRef.tb_uart_tx__DOT__dut__DOT__next_data_reg 
                    = vlSelfRef.tb_uart_tx__DOT__tx_data;
            }
        }
    }
}

VL_ATTR_COLD void Vtb_uart_tx___024root___eval_triggers__stl(Vtb_uart_tx___024root* vlSelf);

VL_ATTR_COLD bool Vtb_uart_tx___024root___eval_phase__stl(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root___eval_phase__stl\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Init
    CData/*0:0*/ __VstlExecute;
    // Body
    Vtb_uart_tx___024root___eval_triggers__stl(vlSelf);
    __VstlExecute = vlSelfRef.__VstlTriggered.any();
    if (__VstlExecute) {
        Vtb_uart_tx___024root___eval_stl(vlSelf);
    }
    return (__VstlExecute);
}

#ifdef VL_DEBUG
VL_ATTR_COLD void Vtb_uart_tx___024root___dump_triggers__act(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root___dump_triggers__act\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Body
    if ((1U & (~ vlSelfRef.__VactTriggered.any()))) {
        VL_DBG_MSGF("         No triggers active\n");
    }
    if ((1ULL & vlSelfRef.__VactTriggered.word(0U))) {
        VL_DBG_MSGF("         'act' region trigger index 0 is active: @(posedge tb_uart_tx.clk)\n");
    }
    if ((2ULL & vlSelfRef.__VactTriggered.word(0U))) {
        VL_DBG_MSGF("         'act' region trigger index 1 is active: @(negedge tb_uart_tx.rst_n)\n");
    }
    if ((4ULL & vlSelfRef.__VactTriggered.word(0U))) {
        VL_DBG_MSGF("         'act' region trigger index 2 is active: @([true] __VdlySched.awaitingCurrentTime())\n");
    }
    if ((8ULL & vlSelfRef.__VactTriggered.word(0U))) {
        VL_DBG_MSGF("         'act' region trigger index 3 is active: @( tb_uart_tx.tx_done)\n");
    }
}
#endif  // VL_DEBUG

#ifdef VL_DEBUG
VL_ATTR_COLD void Vtb_uart_tx___024root___dump_triggers__nba(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root___dump_triggers__nba\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Body
    if ((1U & (~ vlSelfRef.__VnbaTriggered.any()))) {
        VL_DBG_MSGF("         No triggers active\n");
    }
    if ((1ULL & vlSelfRef.__VnbaTriggered.word(0U))) {
        VL_DBG_MSGF("         'nba' region trigger index 0 is active: @(posedge tb_uart_tx.clk)\n");
    }
    if ((2ULL & vlSelfRef.__VnbaTriggered.word(0U))) {
        VL_DBG_MSGF("         'nba' region trigger index 1 is active: @(negedge tb_uart_tx.rst_n)\n");
    }
    if ((4ULL & vlSelfRef.__VnbaTriggered.word(0U))) {
        VL_DBG_MSGF("         'nba' region trigger index 2 is active: @([true] __VdlySched.awaitingCurrentTime())\n");
    }
    if ((8ULL & vlSelfRef.__VnbaTriggered.word(0U))) {
        VL_DBG_MSGF("         'nba' region trigger index 3 is active: @( tb_uart_tx.tx_done)\n");
    }
}
#endif  // VL_DEBUG

VL_ATTR_COLD void Vtb_uart_tx___024root____Vm_traceActivitySetAll(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root____Vm_traceActivitySetAll\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Body
    vlSelfRef.__Vm_traceActivity[0U] = 1U;
    vlSelfRef.__Vm_traceActivity[1U] = 1U;
    vlSelfRef.__Vm_traceActivity[2U] = 1U;
    vlSelfRef.__Vm_traceActivity[3U] = 1U;
}

VL_ATTR_COLD void Vtb_uart_tx___024root___ctor_var_reset(Vtb_uart_tx___024root* vlSelf) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_uart_tx___024root___ctor_var_reset\n"); );
    Vtb_uart_tx__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Body
    const uint64_t __VscopeHash = VL_MURMUR64_HASH(vlSelf->name());
    vlSelf->tb_uart_tx__DOT__clk = VL_SCOPED_RAND_RESET_I(1, __VscopeHash, 4909121462720714774ull);
    vlSelf->tb_uart_tx__DOT__rst_n = VL_SCOPED_RAND_RESET_I(1, __VscopeHash, 14155515402027574779ull);
    vlSelf->tb_uart_tx__DOT__s_tick = VL_SCOPED_RAND_RESET_I(1, __VscopeHash, 18170036037357769263ull);
    vlSelf->tb_uart_tx__DOT__tx_start = VL_SCOPED_RAND_RESET_I(1, __VscopeHash, 1242294580363250427ull);
    vlSelf->tb_uart_tx__DOT__tx_data = VL_SCOPED_RAND_RESET_I(8, __VscopeHash, 12702062035842614992ull);
    vlSelf->tb_uart_tx__DOT__tx = VL_SCOPED_RAND_RESET_I(1, __VscopeHash, 10313645217667034081ull);
    vlSelf->tb_uart_tx__DOT__tx_done = VL_SCOPED_RAND_RESET_I(1, __VscopeHash, 14772283692851637980ull);
    vlSelf->tb_uart_tx__DOT__tick_counter = VL_SCOPED_RAND_RESET_I(4, __VscopeHash, 15592372984424522130ull);
    vlSelf->tb_uart_tx__DOT__dut__DOT__current_state = VL_SCOPED_RAND_RESET_I(2, __VscopeHash, 14400870600160965435ull);
    vlSelf->tb_uart_tx__DOT__dut__DOT__next_state = VL_SCOPED_RAND_RESET_I(2, __VscopeHash, 6338595368939181605ull);
    vlSelf->tb_uart_tx__DOT__dut__DOT__current_count = VL_SCOPED_RAND_RESET_I(4, __VscopeHash, 8286294066291380397ull);
    vlSelf->tb_uart_tx__DOT__dut__DOT__next_count = VL_SCOPED_RAND_RESET_I(4, __VscopeHash, 10725673012375032838ull);
    vlSelf->tb_uart_tx__DOT__dut__DOT__current_bit_count = VL_SCOPED_RAND_RESET_I(3, __VscopeHash, 16020440174141268351ull);
    vlSelf->tb_uart_tx__DOT__dut__DOT__next_bit_count = VL_SCOPED_RAND_RESET_I(3, __VscopeHash, 16604674585782773266ull);
    vlSelf->tb_uart_tx__DOT__dut__DOT__data_reg = VL_SCOPED_RAND_RESET_I(8, __VscopeHash, 9074130253611940023ull);
    vlSelf->tb_uart_tx__DOT__dut__DOT__next_data_reg = VL_SCOPED_RAND_RESET_I(8, __VscopeHash, 13460493714074360147ull);
    vlSelf->tb_uart_tx__DOT__dut__DOT__next_tx = VL_SCOPED_RAND_RESET_I(1, __VscopeHash, 12860265846523179503ull);
    vlSelf->tb_uart_tx__DOT__dut__DOT__next_tx_done = VL_SCOPED_RAND_RESET_I(1, __VscopeHash, 11208910754631886189ull);
    vlSelf->__Vtrigprevexpr___TOP__tb_uart_tx__DOT__clk__0 = VL_SCOPED_RAND_RESET_I(1, __VscopeHash, 3455934708257275634ull);
    vlSelf->__Vtrigprevexpr___TOP__tb_uart_tx__DOT__rst_n__0 = VL_SCOPED_RAND_RESET_I(1, __VscopeHash, 15683414327607292245ull);
    vlSelf->__Vtrigprevexpr___TOP__tb_uart_tx__DOT__tx_done__0 = VL_SCOPED_RAND_RESET_I(1, __VscopeHash, 14832881706164988798ull);
    vlSelf->__VactDidInit = 0;
    for (int __Vi0 = 0; __Vi0 < 4; ++__Vi0) {
        vlSelf->__Vm_traceActivity[__Vi0] = 0;
    }
}
